...
# At the current lime and crop prices, what yield response (t/ha) is required for each field to break even?
breakeven_df <- df_field %>%
  mutate(
    yield_required = (lime_tha * lime_price_base) / crop_price_base,
    profit_y1 = (yield_response * crop_price_base) - (lime_tha * lime_price_base),
    profitable = profit_y1 > 0
  )

ggplot(breakeven_df, aes(x = yield_required, y = yield_response)) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "red") +
  geom_point(aes(color = profitable), alpha = 0.7, size = 2) +
  facet_wrap(~ interaction(crop, admin2_gadm, treatment, sep = " | "), ncol = 3) +
  scale_color_manual(values = c("TRUE" = "#009E73", "FALSE" = "#D55E00"),
                     labels = c("TRUE" = "Profitable", "FALSE" = "Unprofitable")) +
  labs(
    title = "Breakeven Yield Response vs Observed Response",
    subtitle = "Dashed red line: Breakeven yield = (Lime rate × Lime price) / Crop price",
    x = "Breakeven Yield Response (t/ha)",
    y = "Observed Yield Response (t/ha)",
    color = "Field status"
  ) +
  ggthemes::theme_pander(base_size = 14, base_family = "Frutiger") +
  theme(
...
breakeven_summary <- breakeven_df %>%
  group_by(crop, admin2_gadm, treatment) %>%
  summarise(
    avg_yield_required = mean(yield_required, na.rm = TRUE),
    avg_yield_response = mean(yield_response, na.rm = TRUE),
    pct_profitable = mean(profitable, na.rm = TRUE) * 100,
    .groups = "drop"
  )

ggplot(breakeven_summary, aes(x = crop, y = admin2_gadm, fill = avg_yield_required - avg_yield_response)) +
  ...
  facet_wrap(~ treatment) +
  labs(
    title = "Yield Gap: Required vs Observed Yield Response",
    subtitle = "Negative values = on average profitable; Positive = need higher yield response",
    x = "Crop", y = "Site"
  ) +
...
ggplot(breakeven_df, aes(x = treatment, y = yield_required, fill = crop)) +
  geom_boxplot(alpha = 0.6, outlier.shape = NA, width = 0.5) +
  geom_point(aes(y = yield_response), color = "black", alpha = 0.3, size = 1.5, position = position_jitter(width = 0.2)) +
  facet_wrap(~ admin2_gadm, ncol = 3) +
  scale_fill_manual(values = bar_colors) +
  labs(
    title = "Required vs Observed Yield Response for Breakeven (by Treatment)",
    subtitle = "Boxes show breakeven yield (t/ha) — black dots show actual observed responses",
    x = "Lime treatment (t/ha)",
    y = "Yield (t/ha)"
  ) +
  ggthemes::theme_clean(base_size = 14, base_family = "Frutiger") +
  theme(
    strip.background = element_rect(fill = "#1871B8", color = "white"),
    strip.text = element_text(color = "white", face = "bold"),
    legend.position = "bottom"
...
ggplot(breakeven_summary, aes(x = crop, y = admin2_gadm, fill = pct_profitable)) +
  geom_tile(color = "white") +
  facet_wrap(~ treatment, ncol = 3) +
  scale_fill_gradient2(
    low = "#D55E00", mid = "white", high = "#009E73",
    midpoint = 50, name = "% profitable"
  ) +
  labs(
    title = "Share of Fields Above Breakeven Yield (Current Prices)",
    subtitle = "Heatmap of % fields with observed yield ≥ required yield",
    x = "Crop", y = "Site"
  ) +
  theme_minimal(base_size = 14, base_family = my_font_2) +
  theme(
    strip.background = element_rect(fill = "#1871B8", color = "white"),
    strip.text = element_text(color = "white", face = "bold"),
    axis.text.x = element_text(angle = 90, hjust = 1)
  )
ggsave(file.path(output_dir, "breakeven_yield_gap_heatmap.png"),
       width = 10, height = 6, dpi = 500)


cdf_df <- breakeven_df %>%
  group_by(crop, admin2_gadm, treatment) %>%
  arrange(yield_response) %>%
  mutate(cum_pct = 100 * rank(yield_response) / n()) %>%
  ungroup()

ggplot(cdf_df, aes(x = yield_response, y = cum_pct, color = crop)) +
  geom_line(size = 1.1) +
  geom_vline(aes(xintercept = mean(yield_required, na.rm = TRUE)),
             color = "red", linetype = "dashed") +
  facet_wrap(~ admin2_gadm + treatment, ncol = 3) +
  scale_color_manual(values = bar_colors) +
  labs(
    title = "Distribution of Observed Yield Response",
    subtitle = "Dashed red line = mean breakeven yield required for current prices",
    x = "Observed yield response (t/ha)",
    y = "Cumulative % of fields"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    strip.background = element_rect(fill = "#1871B8", color = "white"),
